...
public class Level20_1 {
    ...
    public static void check(int startX, int startY, int n) {
        int startColor = confetti[startX][startY];
        ...
        for(int i = startX; i < startX + n; i++) {
            for(int j = startY; j < startY + n; j++) {
                if(startColor != confetti[i][j]) {
                    check(startX, startY, n/2);
                    check(startX, startY + n/2, n/2);
                    check(startX + n/2, startY, n/2);
                    check(startX + n/2, startY + n/2, n/2);
                    return;
                }
            }
        }
    ...
    public static void addCount(int startColor) {
        if(startColor == BLUE) {
            blueCount += 1;
        }
        else {
